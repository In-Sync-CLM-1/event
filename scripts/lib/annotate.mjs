...
export async function ring(page, locator, { label, below = false } = {}) {
  const box = await locator.boundingBox();
  if (!box) return null;
  const id = '__ann' + (++seq);
  await page.evaluate(({ box, label, id, below }) => {
    const pad = 6;
    ...
    if (label) {
      ...
      l.id = id + 'l';
      l.textContent = label;
      const labelTop = below ? box.y + box.height + pad + 8 : box.y - pad - 34;
      l.style.cssText =
        `position:fixed;left:${box.x - pad}px;top:${labelTop}px;background:#7c3aed;color:#fff;` +
